...
sub markupstring {
	...
	$string =~
	  s#\b(([\w\-_\/]+\.(c|h|cc|cp|hpp|cpp|java|rb))|README)\b#<a href=\"$config->{virtroot}/source$virtp$1\">$1</a>#g;
...
sub markupfile {
	...
	my @ltag = &fileref(1, "line", $pathname, 1) =~ /^(<a.*?)(?:href.*\#)001(\">)1(<\/a>)$/;
	$ltag[0] .= 'id="';
	$ltag[2] .= " ";

	my @itag = &idref(1, "ident", 1) =~ /^(.*=)1(\">)1(<\/a>)$/;
	...
	elsif ($pathname =~ /\.$graphic$/)
	{
		&$outfun("<h3>Image</h3>\n");
		&$outfun("<p><img src=\""
			  . "$config->{virtroot}/source" . $pathname . &urlargs("raw=1")
			  . "\" border=\"0\""
			  . " alt=\"$pathname\">\n");
		&$outfun("</p>");
	}
			...
			&& (length($_) > 132 || /[\000-\010\013\014\016-\037\200-Ÿ]/))
		{
			...
			&$outfun("<h3>Binary File</h3>");

			# jwz: URL-quote any special characters.
			my $uname = $pathname;
			$uname =~ s|([^-a-zA-Z0-9.\@/_\r\n])|sprintf("%%%02X", ord($1))|ge;

			&$outfun("<p>Download link: <a href=\"$config->{virtroot}/source" . $uname . &urlargs("raw=1") . "\">");
			&$outfun("$pathname</a></p>");
...
sub impllinks {
	...
	$current = $config->{virtroot};
	$current =~ s,[^a-z],,;

	foreach $impl ($config->implementations) {
		if ($impl eq $current) {
			$impllink = "<span class=\"var-sel\">$impl</span>";
		} else {
			$impllink =
			    "<a class=\"varlink\" href=\"/$impl/source"
			  . "\">$impl</a>";
		}

		$impls .= expandtemplate($templ, ('impl' => sub { return $impllink }));
	}
	return ($impls);
}
...
sub makeheader {
	...
	print(
		expandtemplate(
			...
			,	'isdiff'     => sub { if($who eq "diff") { return 'diff'; } }
			,	'modes'      => sub { modeexpand(@_,   $who) }
			,	'variables'  => sub { varexpand(@_,    $who) }
			,	'impllinks'  => sub { impllinks(@_) }
